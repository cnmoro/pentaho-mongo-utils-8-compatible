...
class MongoPropToOption {
  ...
  DBObject[] getTagSets( MongoProperties props ) throws MongoDbException {
    ...
    if ( tagSet != null ) {
      ...
      try {
        // com.mongodb.util.JSON is removed in 4.x. Use BasicDBObject.parse logic workaround for list
        DBObject parsed = BasicDBObject.parse( "{ \"items\": " + tagSet + "}" );
        list = (BasicDBList) parsed.get( "items" );
    ...
    if ( !Util.isEmpty( writeConcern ) && Util.isEmpty( wTimeout ) && !journaled ) {
      ...
      if ( !Util.isEmpty( writeConcern ) ) {
        ...
        try {
          ...
          concern = new WriteConcern( wc ).withWTimeout( wt, TimeUnit.MILLISECONDS ).withJournal( journaled );
        } catch ( NumberFormatException n ) {
          // assume its a valid string - e.g. "majority" or a custom
          // getLastError label associated with a tag set
          concern = new WriteConcern( writeConcern ).withWTimeout( wt, TimeUnit.MILLISECONDS ).withJournal( journaled );
        }
      } else {
        concern = new WriteConcern( 1 ).withWTimeout( wt, TimeUnit.MILLISECONDS ).withJournal( journaled );
      }

      if ( log != null ) {
        String lwc =
          "w = " + String.valueOf( concern.getWObject() ) + ", wTimeout = " + concern.getWTimeout( TimeUnit.MILLISECONDS ) + ", journaled = "
            + concern.getJournal();
